...
if (isset($_GET['search'])){
	if (isset($_GET['drugName']) && strlen($_GET['drugName']) > 0)
		$search = strtoupper(str_replace(" ", "%20", $_GET['drugName'])) . '/';
	else
		$search = '0/';
	if (isset($_GET['substanceName']) && strlen($_GET['substanceName']) > 0)
		$search = $search . strtoupper(str_replace(" ", "%20", $_GET['substanceName'])) . '/';
	else
		$search = $search . '0/';
	if (isset($_GET['manufacturerName']) && strlen($_GET['manufacturerName']) > 0)
		$search = $search . strtoupper(str_replace(" ", "%20", $_GET['manufacturerName'])) . '/';
	else
		$search = $search . '0/';
	echo $search;
